 ...
 * Annabelle Huang
 * April 30th, 2023
 * Contains: Implementation of Index for CPSC 223
...
/*
Purpose: builds the index
Input: directory node pointer
Output: none
*/
void Index::build_index(DirNode *node)
{
    auto p = node->get_path(); //get path to node
    // go through all files, call parse file on every file 
    for (int i = 0 ; i < node->num_files() ; i++) { 
        parse_file(p / node->get_file(i));
    }
    // go thorugh every folder in node and recursively call build_index
    for (int j = 0 ; j < node->num_subdirectories() ; j++) { 
        build_index(node->get_subdirectory(j));
    }
}
